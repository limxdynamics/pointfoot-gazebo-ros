...
static limxsdk::PointFootSim *wl;
...
namespace pointfoot_gazebo
{
                          ...
                          const urdf::Model *urdf_model, std::vector<transmission_interface::TransmissionInfo> transmissions)
    {
        ...
        wl = limxsdk::PointFootSim::getInstance();
        wl->init("127.0.0.1");

        // Write the number of motors in the robotCmdBuffer_
        robotCmdBuffer_.writeFromNonRT(limxsdk::RobotCmd(wl->getMotorNumber()));

        // Subscribe to robot command messages and write them to robotCmdBuffer_
        wl->subscribeRobotCmdForSim([this](const limxsdk::RobotCmdConstPtr& msg) 
        {
    ...
    void PointFootHWSim::readSim(ros::Time time, ros::Duration period)
    {
        ...
        if (joint_names_.size() == wl->getMotorNumber()) 
        {
            ...
            if (imuDatas_.size() > 0) 
            {
                ...
                state.imu_acc[0] = imu.linearAcc_[0]; // X
                state.imu_acc[1] = imu.linearAcc_[1]; // Y
                state.imu_acc[2] = imu.linearAcc_[2]; // Z
                state.imu_quat[1] = imu.ori_[0]; // X
                state.imu_quat[2] = imu.ori_[1]; // Y
                state.imu_quat[3] = imu.ori_[2]; // Z
                state.imu_quat[0] = imu.ori_[3]; // W
                state.imu_gyro[0] = imu.angularVel_[0]; // X
                state.imu_gyro[1] = imu.angularVel_[1]; // Y
                state.imu_gyro[2] = imu.angularVel_[2]; // Z
            }
            wl->publishRobotStateForSim(state);
        }
        else
        {
            ROS_ERROR("joint_names size(%ld) != RobotMotorNumber(%u)", joint_names_.size(), wl->getMotorNumber());
        }
